This is a [Next.js](https://nextjs.org) project bootstrapped with [`create-next-app`](https://nextjs.org/docs/app/api-reference/cli/create-next-app).

## Getting Started

Instalação

```bash
npx create-next-app@latest my-site
```

First, run the development server:

```bash
npm run dev
```

Open [http://localhost:3000](http://localhost:3000) with your browser to see the result.

You can start editing the page by modifying `app/page.js`. The page auto-updates as you edit the file.

# Tópicos Importantes

- O que é React?
- O que é Next.js?
- O que é Single Page Application (SPA)?
- Como instalar o Next.js?
- Qual a diferença entre os pacotes react, react-dom e next?
- O que são Componentes?
- O que é JSX?
- Como estilizar um componente?
- Como adicionar Imagens com Next.js?
- Para que serve a biblioteca React Icons?
- O que são Hooks?
- Como funciona e para que serve o useState?
- Como funciona e para que serve o useEffect?
- O que são propriedades dos componentes?
- Prop Drilling

### Tópicos Avaliação 2

- Navegação (Next.js)
  - Diferença de navegar com componente Link ou useRouter vs Tag <a>
  - Como ler parâmetros de url ex: ?id=10&name=Renan (useSeachParams)  
- Virtual DOM
  - Como funciona o virtual DOM do React? 
- Estados Globais (Zustand)
- Server Components
- CRUD

-------

# Avalições:

Prova 1 (peso 50)
Prova 2 (peso 30)
Apresentação (peso 20)

-------

# Apresentação:

- De 1 a 4 integrantes
- Cada integrante deve apresentar 10 - 15 minutos.
- Tema deve ser validado com professor.
- Agendar data de aprensentação.

-------

# Temas:

- React Query (Tanstack) - Vitor
- Chart.js - Gráficos - Rafael Tomaz
- Redux
- Axios - Tiago Venancio
- Material UI (Componentes de UI) - Letícia Pacheco, Felipe Cursino e Mauricio
- Shadcn (Componentes de UI) - João Gabriel e Lucas
- Mantine (Componentes de UI) - Arthur e Vinícius
- Base UI (Componentes de UI) Emerson Soares
- React Uploady - Kevin, Ana, Thiago Carvalho
- Formik (Formulários)
- React Hook Form (Formulários) - Eduarda
- Framer Motion - Vitor Ramos
- React Game Engine - Kauan Leonel
- Mapas (Google Maps React / React Leaflet)
- Teste de Componentes
- React Test Library - Nicoli / Giselle / Raissa / David 

(Aplicações de IA para React)
- Skills para React

Data Apresentações:

- 18/05
- 25/05
- 01/06
- 08/06
- 15/06
- 22/06

